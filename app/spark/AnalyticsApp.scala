...
object AnalyticsApp {
  ...
  def main(args: Array[String]) {
    ...
    val logData = sc.textFile(s"s3n://$awsCredentialFragment@$bucketName/$inputPattern/*")
    ...
    val somedata = webEventDataByHourAndSite.map(x=>(x._1._1._1, x._1._1._2,x._1._1._3,x._1._1._4, x._1._2, x._2.groupBy(x=>x.id).keys.size))
    ...
    webEventDataByHourAndSite.map(x=>(x._1,
      ...
      case None => "direct"
      case Some(y) => y
    }).groupBy(x=>x).map(x=>(x._1,x._2.size)))).flatMap(x=>x._2.map(y=>(x._1, y._1, y._2)))
